...
function redirect2Lang(){
	$('.language-switcher a').on('click', function(){
		localStorage.selectedLang = $(this).attr('href')
	})
	...
	if (!localStorage || localStorage.redirected2Lang || localStorage.selectedLang) {
		return
	}


	var redirect = function(lang){
		localStorage.redirected2Lang = lang
		...
		if (lang!='en' && window.location.href.indexOf(langs_pages[lang]) > -1) {
			return
		}
		...
		console.log('redirect', lang, langs_pages[lang])
