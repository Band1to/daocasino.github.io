...
var modules = {
    ...
    build       : function() {
        ...
        var taskBuildCopy = function() {
            return gulp.src(['**', '!~QA/**', '!partials/**', '!package.json', '!settings.json', '!peon.json', '!drone.json', '!README.md', '!gulpfile.js', '!' + config.paths.source.sprite + '/*', '!' + config.paths.source.css + '/*', '!**/*.map', '!build', '!*.html', '*.pdf', '*.ico'])
                .pipe(copy('build/'));
        };
...
gulp.task('build', ['build:clean'], function() {
    ...
    runSequence('images:resize-sprite-source', 'css:lazy-rules', 'css:import-modules', 'css:compile', 'images:sprites', 'build:copy', 'build:include');
});
